...
def query_chemical_agent(query, mode, api_key, history=[]):
    # 🟢 获取数据库的推荐结果（不管对错，先拿出来）
    results = db_collection.query(query_texts=[query], n_results=1)
    matched_name = results['metadatas'][0][0]['name'] if results['metadatas'][0] else "无"
    matched_smiles = results['metadatas'][0][0]['smiles_list'] if results['metadatas'][0] else "无"

    # 🟢 剥夺 Python 直接画图的权力，清空画板！
    st.session_state.matched_smiles = None

    # 🟢 强迫大模型自己去当裁判
    # 🟢 强迫大模型自己去当裁判，并强制输出机器标签
    rag_context = f"""
        [系统提示：本地知识库推荐] 
        反应名称: {matched_name} 
        图纸代码: {matched_smiles}

        ⚠️ 最高优先级指令（用于触发后端3D引擎，绝对不可违反）：
        无论你作何解答，你【必须】在回答的绝对最后一行，输出带有 <SMILES> 和 </SMILES> 的机器读取标签！
        1. 如果用户的提问与推荐反应【匹配】：请将上面的“图纸代码”原封不动地抄写在标签内。格式必须是：<SMILES>{matched_smiles}</SMILES>
        2. 如果风马牛不相及（错配）：请彻底无视推荐数据！你自己写出正确核心产物的 SMILES，并包裹在标签内输出。例如：<SMILES>CC=O</SMILES>

        严禁在 <SMILES> 标签内进行任何 Markdown 格式化！丢失此标签将导致系统严重崩溃！
        """
    base_prompt = f"""
    {rag_context}
...
if st.session_state.matched_smiles and len(st.session_state.matched_smiles) > 0:
    ...
    # 🌟 智能滑块：如果只有 1 个分子，就隐藏滑块直接展示；如果有多个，才显示滑块
    if steps > 1:
                ...
                if not api_key:
                    ...
                    with st.chat_message("assistant"):
                        with st.spinner("AI 正在解析多维数据并调取文献..."):
                            ...
                            # 🌟 新增的兜底提取魔法：如果数据库没给图纸，就从 AI 回答里抓取！
                            # 🌟 核心修改：去掉 if not 条件！只要 AI 输出了 <SMILES>，就说明它发起了纠错，直接用它的图纸覆盖掉数据库的垃圾图纸！
                            # 🌟 终极图纸提取引擎：无论 AI 吐出什么格式，全部拿下
                            smiles_match = re.search(r'<SMILES>(.*?)</SMILES>', raw_reply, re.IGNORECASE | re.DOTALL)
                            if smiles_match:
                                dynamic_smiles = smiles_match.group(1).strip()
                                try:
                                    import json

                                    # 尝试将 AI 抄回来的复杂图纸转化为 Python 列表
                                    clean_smiles = dynamic_smiles.replace("'", '"')
                                    parsed_smiles = json.loads(clean_smiles)
                                    if isinstance(parsed_smiles, list):
                                        st.session_state.matched_smiles = parsed_smiles
                                    else:
                                        st.session_state.matched_smiles = [dynamic_smiles]
                                except:
                                    # 如果是 AI 自己写的单个分子（如 CC=CC=O），直接装进列表
                                    st.session_state.matched_smiles = [dynamic_smiles]

                                # 把丑陋的标签从屏幕上抹掉
                                raw_reply = re.sub(r'<SMILES>.*?</SMILES>', '', raw_reply,
